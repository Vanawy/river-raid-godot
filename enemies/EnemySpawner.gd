...
func spawn_jet(global_pos: Vector2, level: Level, remove_random: bool = false) -> void:
		...
		enemy.destroyed.connect(_enemy_killed.bind(enemy, 100))
...
func spawn_aa_boat(global_pos: Vector2, level: Level, difficulty: float) -> void:
		...
		enemy.destroyed.connect(_enemy_killed.bind(enemy, 300))
	
func _enemy_killed(enemy: Node2D, score: int) -> void:
	enemy_killed.emit(score, enemy.global_position)
